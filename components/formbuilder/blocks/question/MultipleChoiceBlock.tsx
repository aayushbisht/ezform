...
const MultipleChoiceBlock: React.FC<MultipleChoiceBlockProps> = ({ block, onChange }) => {
  const addOption = () => {
    if (block.options) {
      ...
      <div className="space-y-2">
        {block.options?.map((option, index) => (
          <div key={index} className="flex items-center">
            <div className="mr-2 w-4 h-4 rounded-full border border-gray-400 flex-shrink-0"></div>
            <input
              type="text"
              className="flex-1 p-2 border-b border-transparent focus:border-gray-300 focus:outline-none"
              value={option}
